...
- PR #174 hardens client→server authority and payload boundaries across villager editor/data-fetch, interaction, family call, global custom-clothing, village/building and related handlers. Baby naming now accepts only an unnamed `BabyItem` (including specialized subclasses), so a forged packet cannot grant a free custom name to an arbitrary inventory item or rename a baby after the one-time naming flow completed.
- PR #174 adds explicit permanent-removal cleanup for the five UUID-keyed Living World stores through `NpcRemovalLifecycle`, while preserving the same persistent NPC identity across tombstone capture/resurrection: tombstone-captured deaths keep episodic/semantic memory, NPC social state, player relationship state and persistent voice assignment; only permanent deaths purge them.
- PR #174 fixes family-tree cleanup on death: removing an unremembered child clears that child's parent links instead of deleting the living parents' complete family-tree nodes.
- PR #174 also hardens long-lived runtime/delivery state: bounded/invalidation-aware client caches, per-call SHA-256 digesting instead of shared mutable digest state, governed exact-line security-scan exceptions, validated artifact labels and corrected Fabric/NeoForge loader/runtime version metadata.
...
- Release-convergence governance gap, found while landing the capacity-index slice: PR #172 was the first `feat:`-prefixed commit merged after `docs/releases/0.3.0-convergence.json` was frozen (PR #160), which broke `test_repository_contract_matches_current_release_boundary`'s history-vs-contract check in the `VillAIgence GitHub Release` workflow. `0.3.0-convergence.json` is a closed historical contract — its already-published `[0.3.0+1.21.1]` changelog section can never honestly reference a PR that shipped after that release. Fixed by creating `docs/releases/0.4.0-convergence.json` (same schema as PR #160, `previousRelease=0.3.2+1.21.1`, `candidateTag=0.4.0+1.21.1` planned but not armed — `NEXT_RELEASE.txt` stays `0.3.2+1.21.1`, mirroring how `0.3.0-convergence.json` coexisted with `NEXT_RELEASE.txt` still at `0.2.0+1.21.1`), `capabilityPullRequests: [172]`, and repointing `scripts/ci/test_release_convergence.py`'s `CONTRACT_PATH` at it. Going forward, every new `feat:`-prefixed capability PR must add its PR number to `docs/releases/0.4.0-convergence.json`'s `capabilityPullRequests` in the same PR, alongside the existing `changelog.md` `[Unreleased]` update policy; `fix:`/`perf:`/`docs:`/`release:` commits are unaffected.
- PR #178 closes a second release-convergence edge case exposed by enforcing that same policy during an unmerged feature PR. Pull-request release validation still reads feature history from the real base branch rather than GitHub's synthetic merge commit, but may now include exactly the current PR as a pending capability only when GitHub event metadata proves the event is `pull_request`, its title is `feat:`-prefixed and its PR number is a positive decimal. Push/tag validation receives no pending capability and therefore still requires every capability PR to exist physically in post-release git history. This lets a feature PR honestly register itself in `0.4.0-convergence.json` before merge without weakening post-merge/tag convergence checks.


### Validation

- PR #178 used staged RED→GREEN TDD for social route preference and same-cycle anti-retargeting. The first RED run (VillAIgence CI #3008, run `35395435641`) executed 865 common tests with exactly one intended failure: `positiveDirectedRouteIsPreferredOverLegacyNeutralTarget`. After the minimal bounded routing policy/lifecycle integration, common tests passed. A second RED (CI #3037, run `35396260246`) executed 866 tests with exactly one intended failure: `sameCycleSocialChangeCannotRetargetSuccessfulSourceToSecondListener`. The first guard attempt was deliberately rejected when CI #3039 exposed one preservation regression in `laterCycleMayProgressToAnotherDeterministicTargetWithoutBroadcast`; the corrected cycle-scoped guard then passed exact-head Repository security #2677, VillAIgence CI #3042, Production Soak #598 and GitHub Release dry-run #934 before the final characterization/bookkeeping pass. Release dry-run #946 later exposed the unmerged-feature convergence mismatch (`contract=(172,178), history=(172,)`). A tests-first pending-feature context then produced the intended contract RED on missing `resolve_current_feature_pr`, and release dry-run #952 subsequently passed `Run production acceptance contract tests` with base history plus the verified pending `feat:` PR while preserving strict push/tag history semantics.
