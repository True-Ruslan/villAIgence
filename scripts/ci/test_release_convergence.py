...
class ReleaseConvergenceValidatorTest(unittest.TestCase):
    ...
    def test_post_release_capability_inventory_is_exactly_172_and_177(self) -> None:
        contract = load_contract(REPOSITORY_ROOT / CONTRACT_PATH)
        self.assertEqual((172, 177), tuple(contract["capabilityPullRequests"]))
...
"""
        ...
        self.assertTrue(
            any(
                "release section 0.4.0+1.21.1 does not reference capability PR #177"
                in error
                for error in errors
            )
        )
