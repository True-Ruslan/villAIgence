...
2. Look toward that villager and use your normal Simple Voice Chat microphone / push-to-talk key.
3. Stop speaking briefly. After roughly 800 ms of microphone inactivity, the utterance is finalized.
4. The server verifies that the selected NPC is still nearby, visible, and in your view before sending audio to STT.
5. The server runs STT, sends the transcript through the existing MCA/LivingWorld ChatAI context, synthesizes the answer, and plays it spatially from the NPC.
6. The answer is also added to the existing MCA conversation text path.
...
`microphone -> Simple Voice Chat -> Opus decode -> target validation -> 48 kHz PCM -> WAV -> OpenAI STT -> MCA ChatAI -> OpenAI TTS WAV -> 48 kHz PCM -> Simple Voice Chat spatial NPC audio`

Raw microphone audio is processed in memory and is not intentionally persisted by LivingWorld.

## Troubleshooting

- No NPC reaction: verify you interacted with the NPC first, are looking toward it while speaking, and that `apiKey` or `OPENAI_API_KEY` is set.
