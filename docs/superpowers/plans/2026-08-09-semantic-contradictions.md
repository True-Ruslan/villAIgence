...
- Modify after RED: `MemoryEvent.java`, `MemoryEventRetentionPolicy.java`
...
- [ ] Implement the minimal record, MemoryEvent field/constructors and retention switch branch.
...
- [ ] RED: mutate owner/type/provenance/gameTime/summary/snapshot claim ID/entry ID/kind/provenance/statement/scope and prove `valid` fails.
...
- [ ] Implement exact event filtering and current Semantic claim lookup by stable logical identity. Do not parse summaries.
- [ ] Run focused/full common GREEN.
...
- [ ] Update root `[Unreleased]`: structured contradiction process evidence, exact-ID authority, stable logical identity, no truth promotion, query only while claims remain live, no prompt/provider/config/store/migration change.
...
- [ ] Independent base→head review checks provider/client authority, no summary parsing, no claim mutation, no truth winner, no resurrection after forgetting, consolidation compatibility, bounded retention, privacy, persistence versions and existing rumor invariants.
...
Spec coverage: all persistence, identity, lifecycle, query, privacy, forgetting, truth and delivery requirements map to Tasks 1-7.
...
Scope remains one cohesive representation/lifecycle/query slice. Automatic contradiction detection, prompt injection, uncertainty, distortion and trust weighting are explicitly excluded.