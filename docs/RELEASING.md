...
Latest published and installed-accepted release:
...
## 0.3.0 rollout boundary

`0.3.0` packages the bounded post-`0.2.0` Memory 2.0 capability set plus MCA Personality / directed NPC↔NPC social state through PR #158.
...
Canonical post-publication installed plan:
...
4. validate the machine-readable 0.3 convergence contract and exact changelog/release-request state;
...
8. stage the exact remapped Fabric candidate with `-Prelease_version=0.3.0+1.21.1`;
...
## Expected `0.3.0` assets

```text
villaigence-fabric-0.3.0+1.21.1.jar
villaigence-fabric-0.3.0+1.21.1.jar.sha256
villaigence-dependencies-0.3.0+1.21.1.txt
```
...
1. stop the server;
2. retain the current known-good `0.2.0+1.21.1` JAR;
3. make an offline backup of the test world, `config/` and `mods/`;
4. preserve any previous `<world>/livingworld/` separately;
5. use a clean LivingWorld test state for 0.3;
6. record the official 0.3 release JAR SHA-256 before installation.
...
6. use a new release version if the failing 0.3 tag has already been published and runtime bytes must change.
